"""Full-denominator aggregation of formal FQ3 diagnostics."""
...
def main():
    ...
    ap.add_argument("--reports", type=Path, default=Path("results/sequence_ml_fq3_v1"))
    ap.add_argument("--fq2-summary", type=Path, default=Path("results/sequence_ml_fq2_v1/fq2_summary.json"))
    ap.add_argument("--out", type=Path, required=True)
    args = ap.parse_args()
    base_path, q3_path = Path("configs/sequence_ml_fq2_v1.json"), Path("configs/sequence_ml_fq3_v1.json")
    ...
    for month in months:
        for symbol in symbols:
            path = args.reports / f"fq3_update_{month}_{symbol}.json"
            r = json.loads(path.read_text())
            if r["symbol"] != symbol or r["evaluation_month"] != month or r["fq3_config_sha256"] != sha(q3_path):
    ...
    diagnostic_reports = {}
    for symbol in symbols:
        path = args.reports / f"fq3_diagnostic_{symbol}.json"
        r = json.loads(path.read_text())
        if r["symbol"] != symbol or r["fq3_config_sha256"] != sha(q3_path):
    ...
    summary = {"stage": q3["stage"], "retrospective_only": True,
               "fq3_config_sha256": sha(q3_path), "fixed_runner_commit": json.loads(args.fq2_summary.read_text())["source_commit"],
               ...
               "input_update_hashes": {f"{month}/{symbol}": sha(args.reports/f"fq3_update_{month}_{symbol}.json") for month,symbol in update_reports},
               "input_diagnostic_hashes": {symbol: sha(args.reports/f"fq3_diagnostic_{symbol}.json") for symbol in symbols},
    ...
    if args.out.exists():
        raise ValueError("preserve existing FQ3 summary")
