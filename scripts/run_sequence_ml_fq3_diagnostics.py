"""FQ3 past-activity and visible crossing on fixed FQ2 200k event rows."""
...
def main():
    ...
    config["training_endpoints_per_stock_max"] = q3["formal_size_per_stock"]
    ...
    q2 = json.loads(args.fixed_report.read_text())
    if (q2["stage"], q2["symbol"], q2["training_size_per_stock"], q2["config_sha256"]) != ("FQ2", args.symbol, q3["formal_size_per_stock"], sha(base_path)):
        raise ValueError("fixed FQ2 report identity mismatch")
    private_path = args.private / "predictions.npz"
    if sha(private_path) != q2["private_predictions_sha256"]:
    ...
    if len(day) != q2["selection_counts"]["eval"]:
        raise ValueError("FQ2 prediction denominator changed")
    ...
    for date in np.unique(day):
        ...
        if not np.array_equal(frame.event_index.to_numpy()[endpoints], event[ids]):
            raise ValueError(f"FQ2 event row mismatch: {args.symbol}/{date}")
        ...
        if not np.array_equal(actual, y[ids]):
            raise ValueError(f"FQ2 target row mismatch: {args.symbol}/{date}")
    ...
    report = {"stage": q3["stage"], "symbol": args.symbol, "retrospective_only": True,
              "fq3_config_sha256": sha(q3_path), "base_config_sha256": sha(base_path),
              "diagnostic_script_sha256": sha(Path(__file__)),
              "crossing_library_sha256": sha(Path("src/cloblab/q3_diagnostics.py")),
              "source_commit": subprocess.check_output(["git", "rev-parse", "HEAD"], text=True).strip(),
              "cache_manifest_sha256": sha(manifest_path), "fixed_prediction_sha256": sha(private_path), "fixed_fq2_report_sha256": sha(args.fixed_report),
