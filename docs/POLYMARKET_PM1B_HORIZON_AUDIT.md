...
## Private train/dev target build after horizon freeze

The fixed builder `scripts/build_polymarket_pm1b.py` matched the outcome-blind coverage audit exactly: train **392 markets / 12,479 opportunities** and dev **234 / 7,476**. It uses the same private PM1A causal decision/history arrays, verifies their hashes, reads only public book states (no terminal label fields), applies the strict future lookup and writes probability-point repricing targets to private `.npz` files. Public train/dev manifests in `results/polymarket_pm1b_v1/` contain counts, exclusion causes, source/config hashes and private array hashes, never target values or row identifiers. The final repricing target has not been built. This is preprocessing evidence, not a PM1B predictive result.
