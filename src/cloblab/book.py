...
class L2Book:
    ...
    def to_snapshot_row(self) -> dict[str, object]:
        ...
        for level, (price_ticks, size_lots) in enumerate(sorted(self.bids.items(), reverse=True)[:10], start=1):
            row[f"bid_px_{level}"] = _from_units(price_ticks, self.tick_size)
            row[f"bid_sz_{level}"] = _from_units(size_lots, self.lot_size)
        for level, (price_ticks, size_lots) in enumerate(sorted(self.asks.items())[:10], start=1):
            row[f"ask_px_{level}"] = _from_units(price_ticks, self.tick_size)
            row[f"ask_sz_{level}"] = _from_units(size_lots, self.lot_size)
        return row

    def state_hash(self) -> str:
        encoded = repr((sorted(self.bids.items()), sorted(self.asks.items()))).encode("utf-8")
